...
const navItems = [
    ...
    {
        label: 'Friendship Analysis',
        href: '/friendship-analysis',
        icon: (
            <Heart className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} />
        ),
    },
    {
            ...
            >
                ...
                <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                    <Link href="/" className="flex items-center gap-3" onClick={onClose}>
                        ...
                        <div>
                            <h1 className="text-slate-900 font-semibold text-lg leading-tight">Hans AI</h1>
                ...
                <div className="p-4 border-t border-slate-100">
                    <div className="flex items-center gap-2 px-4 py-2">
                        ...
                        <span className="text-xs text-slate-500 font-medium">Gateway Online</span>
                    </div>
                </div>
            </aside>

            {/* ── Mobile bottom navigation bar ── */}
            <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur-sm border-t border-slate-200 flex md:hidden h-16 shadow-lg shadow-black/5">
                {navItems.map((item) => {
                    const isActive = pathname === item.href || pathname?.startsWith(item.href + '/');
                    return (
                        <Link
                            ...
                            href={item.href}
                            className={cn(
                                'flex-1 flex flex-col items-center justify-center py-2 gap-1 text-[11px] font-medium transition-colors',
